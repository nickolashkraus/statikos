...
.PHONY: test tox unit-test coverage lint check format
.DEFAULT_GOAL := help
...
test: install unit-test coverage lint check ## run unit-test, coverage, lint, and check

tox: ## run tests on specific Python versions with tox
	tox
...
browser: ## run code coverage and open report in browser
	@echo 'Running coverage...' && \
	nosetests . && exit 0 || \
	coverage report -m >/dev/null || coverage html; \
	$(BROWSER) htmlcov/index.html
...
check: ## check complexity and docstrings
	...
	failed=0 && \
	for f in $$(find $$(pwd)/statikos/ -name '*.py';); do \
...
release: dist ## package and publish Python package
	twine upload dist/*
...
install: clean ## install the package to the activated Python site-packages
